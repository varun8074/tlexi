...
from ..examples.response_example import (
    success_response,
    no_data,
    server_error,
    d_success_response,
...
@router.get(
    ...
    responses={
        200: {
            ...
            "content": {"application/json": {"example": d_success_response}},
        },
        500: {
            "description": "internal server error",
            "content": {"application/json": {"example": d_server_error}},
        },
    },
)
async def load_data():
...
@router.post(
    ...
    responses={
        200: {
            "description": "A successful response",
            "content": {"application/json": {"example": success_response}},
        },
        404: {
            "description": "Answer Not Found",
            "content": {"application/json": {"example": no_data}},
        },
        500: {
            "description": "internal server error",
            "content": {"application/json": {"example": server_error}},
        },
    },
)
    ...
    try:
        ...
        if not retriever or not qa_pipeline:
            return JSONResponse(
                status_code=500,
                content={
                    "error": "Run /load_data first to initialize retriever and qa_pipeline"
                },
            )

        response = ask_question(query.query, retriever, qa_pipeline)

        if not response or "answer" not in response or not response["answer"].strip():
            return JSONResponse(
                status_code=404, content={"error": "No relevant answer found"}
            )
